...
class CDROMDevice(Device):
    ...
    def validate_impl(self) -> list[tuple[str, str]]:
        ...
        if not self.path.strip():
            verrors.append(('attributes.path', 'Path is required for CDROM device'))
        elif not os.path.isabs(self.path):
            verrors.append(('attributes.path', 'Path must be an absolute path'))
        elif not os.path.exists(self.path):
            verrors.append(('attributes.path', f'Path {self.path} does not exist'))
        return verrors
