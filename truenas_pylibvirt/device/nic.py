...
class NICDevice(Device):
    ...
    def validate_impl(self) -> list[tuple[str, str]]:
        ...
        if self.source and self.source.startswith('br') and self.trust_guest_rx_filters:
            verrors.append(
                ('attributes.trust_guest_rx_filters', 'This can only be set when "nic_attach" is not a bridge device')
            )

        if self.trust_guest_rx_filters and self.model == NICDeviceModel.E1000:
            verrors.append(
                ('attributes.trust_guest_rx_filters', 'This can only be set when "type" of NIC device is "VIRTIO"')
            )

        if self.mac and self.mac.lower().startswith('ff'):
            verrors.append(
                ('attributes.mac', 'MAC address must not start with `ff`')
            )
        return verrors
