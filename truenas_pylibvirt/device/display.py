...
class DisplayDevice(Device):
    ...
    def validate_impl(self) -> list[tuple[str, str]]:
        ...
        if not self.password or not self.password.strip():
            verrors.append(('attributes.password', 'Password is required for display devices'))

        if self.type_ == DisplayDeviceType.VNC:
            if self.web:
                verrors.append(
                    ('attributes.web', 'Web access is not supported for VNC display devices, please use SPICE instead')
                )
            if self.password and len(self.password) > 8:
                ...
                verrors.append(
                    ('attributes.password', 'Password for VNC display devices must be 8 characters or less')
                )
        elif self.type_ == DisplayDeviceType.SPICE:
            if self.port == self.web_port:
                verrors.append(
                    ('attributes.port', 'Spice server port must not be same as web port')
                )
        return verrors
