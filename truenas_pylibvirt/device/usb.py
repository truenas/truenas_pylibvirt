...
class USBDevice(Device):
    ...
    def validate_impl(self) -> list[tuple[str, str]]:
        ...
        if self.device and (self.product_id or self.vendor_id):
            verrors.append(
                ("attributes.device", "Either device must be specified or USB details but not both")
            )
        elif not self.device and not (self.product_id or self.vendor_id):
            verrors.append(
                (
                    "attributes.usb",
                    "Either device or attributes.product_id and attributes.product_id  must be specified"
                )
            )
        ...
        if self.device:
            if not usb_device_details:
                verrors.append(("attributes.device", f"No USB device found with name {self.device}"))
            elif usb_device_details.get("error"):
                verrors.append(("attributes.device", usb_device_details["error"]))
        else:
            if not usb_device_details:
                verrors.append(
                    (
                ...
                verrors.append(("attributes.usb", usb_device_details["error"]))
